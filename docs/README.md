...
> ⚠️ **Ограничения**: Текущий драйвер `ptp_ocp` предоставляет ограниченный набор метрик. Детальный мониторинг температуры, питания, GNSS созвездий и расширенные PTP метрики пока недоступны. См. [ROADMAP](quantum-pci-monitoring/ROADMAP.md) для планов развития.
