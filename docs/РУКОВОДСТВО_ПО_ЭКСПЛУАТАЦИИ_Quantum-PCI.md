...
| № | Наименование | Кол. |
|---|--------------|------|
| 1 | Кабель антенный соединительный РК 50-7-311 с установленными разъёмами N – N (длина определяется при заказе) | 1 |
| 2 | Кабель антенный соединительный РК 50-7-11 с установленными разъёмами N – N (длина определяется при заказе) | 1 |
| 3 | Кабель антенный соединительный РК 50-4,8-32 с установленными разъёмами N – N (длина определяется при заказе) | 1 |
...
**Таблица 2.2 - Коды заказа по типу опорного генератора:**

| Наименование | Тип опорного генератора | Описание |
|--------------|-------------------------|----------|
| **QUANTUM-Q-01** | Кварцевый генератор | Уход времени в автономном режиме не более ±1.8мкс в сутки (стандартная поставка) |
| **QUANTUM-Q-02** | Кварцевый генератор | Уход времени в автономном режиме не более ±22мкс в сутки (экономичная поставка) |
| **QUANTUM-Q-03** | Высокоточный кварцевый генератор | Уход времени в автономном режиме не более ±1.5мкс в сутки (премиум поставка) |
| **QUANTUM-R-01** | Рубидиевый стандарт | Уход времени в автономном режиме не более ±0.3мкс в сутки (поставка под заказ) |
| **QUANTUM-C-01** | Цезиевый стандарт | Уход времени в автономном режиме не более ±0.1мкс в сутки (поставка под заказ) |
...
- Вход/выход сигналов точного времени (1PPS и 10MHz) через SMA‑разъёмы
...
### 4.16.1. Настройка прав доступа для PTP устройств
...
### 4.16.2. Выбор правильного PTP устройства
...
### 4.16.3. Диагностика проблем с PHC в chrony
...
### 4.18 Эксплуатация и обслуживание
...
### 4.19 Интеграция с различными системами
...
### 4.20 Расширенное устранение неполадок
...
### 4.21 Производительность и оптимизация
...
### 4.22 Тестирование и валидация

#### Бенчмарки и тестирование

### 4.22.1 Тест автономного хранения 1PPS (24 часа)
...
### 4.22.2 Тест автономного хранения 1PPS (24 часа) - расширенный
...
### 4.22.3 Тест PTP с одним клиентом (phc2sys) - внутренняя антенна
...
### 4.22.4 Тест PTP с одним клиентом (phc2sys) - внешняя антенна
...
### 4.22.5 Тест PTP с одним клиентом (ts2phc) - внутренняя антенна
...
### 4.22.6 Тест PTP с одним клиентом (ts2phc) - внешняя антенна
...
### 4.22.7 Тестирование PTM (Precision Time Measurement)
...
### 4.23 Безопасность и соответствие стандартам
...
### 4.24 Консолидированные инструкции из каталога docs
...
### 6.5 Расширенное тестирование с осциллографом

**Дополнительные возможности осциллографических измерений:**

Помимо базового тестирования сигналов 10 МГц и 1PPS, Quantum-PCI позволяет проводить более детальный анализ временных характеристик с помощью осциллографа.

#### 6.5.1 Измерение джиттера и фазового шума
...
**Параметры осциллографа для измерения джиттера:**
- **Полоса пропускания**: не менее 1 ГГц
- **Частота дискретизации**: не менее 10 ГС/с
- **Временная база**: 1 нс/дел для 10 МГц, 1 мс/дел для PPS
- **Режим измерения**: статистический анализ джиттера
- **Количество измерений**: не менее 1000 для статистической значимости

**Критерии качества сигнала:**
- **Джиттер 10 МГц**: не более 50 пс RMS
- **Джиттер PPS**: не более 500 пс RMS
- **Фазовый шум**: не более -100 дБн/Гц на частоте 1 кГц от несущей

#### 6.5.2 Измерение задержек и синхронизации

**Тестирование синхронизации между выходами:**
```bash
...
**Измерение задержек между выходами:**
- **Максимальная задержка между выходами**: не более 1 нс
- **Стабильность задержки**: не более 100 пс
- **Синхронизация с GNSS**: не более 100 нс

#### 6.5.3 Анализ переходных процессов

**Настройка для анализа переходных процессов:**
```bash
...
# Функция измерения
measure_frequency() {
    # Здесь должна быть команда для измерения частоты осциллографом
    # Для примера используем случайное значение
    echo $(echo "scale=6; 10000000 + (RANDOM-16384)/1000000" | bc)
}
...
echo "Время,Температура,Частота_10MHz,Джиттер" > $LOG_FILE
...
for i in {1..60}; do
    ...
    echo "$timestamp,$temperature,$frequency,$jitter" >> $LOG_FILE
    echo "Измерение $i/60: T=$temperature°C, F=$frequency Гц, J=$jitter нс"
...
**Критерии температурной стабильности:**
- **Температурный коэффициент**: не более 1×10⁻⁶/°C
- **Стабильность в диапазоне 0-50°C**: не более 5×10⁻⁶
- **Гистерезис**: не более 2×10⁻⁶

#### 6.5.5 Документирование результатов измерений

**Шаблон отчета о измерениях:**
```bash
#!/bin/bash
# generate-measurement-report.sh - Генерация отчета о измерениях

REPORT_FILE="/var/log/oscilloscope-measurements-$(date +%Y%m%d).txt"

echo "=== ОТЧЕТ О ОСЦИЛЛОГРАФИЧЕСКИХ ИЗМЕРЕНИЯХ ===" > $REPORT_FILE
echo "Дата: $(date)" >> $REPORT_FILE
echo "Изделие: Quantum-PCI" >> $REPORT_FILE
echo "Серийный номер: $(cat /sys/class/timecard/ocp0/serialnum 2>/dev/null || echo 'N/A')" >> $REPORT_FILE
echo >> $REPORT_FILE

# Информация об оборудовании
echo "=== ИЗМЕРИТЕЛЬНОЕ ОБОРУДОВАНИЕ ===" >> $REPORT_FILE
echo "Осциллограф: [Указать модель и серийный номер]" >> $REPORT_FILE
echo "Генератор сигналов: [Указать модель и серийный номер]" >> $REPORT_FILE
echo "Кабели: [Указать тип и длину]" >> $REPORT_FILE
echo >> $REPORT_FILE

# Результаты измерений
echo "=== РЕЗУЛЬТАТЫ ИЗМЕРЕНИЙ ===" >> $REPORT_FILE
echo "Сигнал 10 МГц:" >> $REPORT_FILE
echo "  Частота: [Указать измеренное значение] Гц" >> $REPORT_FILE
echo "  Амплитуда: [Указать измеренное значение] В" >> $REPORT_FILE
echo "  Джиттер: [Указать измеренное значение] пс RMS" >> $REPORT_FILE
echo "  Скважность: [Указать измеренное значение] %" >> $REPORT_FILE
echo >> $REPORT_FILE

echo "Сигнал 1PPS:" >> $REPORT_FILE
echo "  Период: [Указать измеренное значение] сек" >> $REPORT_FILE
echo "  Амплитуда: [Указать измеренное значение] В" >> $REPORT_FILE
echo "  Джиттер: [Указать измеренное значение] нс RMS" >> $REPORT_FILE
echo "  Длительность импульса: [Указать измеренное значение] мс" >> $REPORT_FILE
echo >> $REPORT_FILE

# Заключение
echo "=== ЗАКЛЮЧЕНИЕ ===" >> $REPORT_FILE
echo "Все измеренные параметры соответствуют техническим требованиям." >> $REPORT_FILE
echo "Изделие Quantum-PCI признано годным к эксплуатации." >> $REPORT_FILE
echo >> $REPORT_FILE
