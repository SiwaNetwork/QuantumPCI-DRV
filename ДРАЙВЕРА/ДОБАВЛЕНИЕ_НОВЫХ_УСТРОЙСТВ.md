# Руководство по добавлению поддержки новых устройств Quantum-PCI TimeCard
...
Для устройств, поддерживающих обновление прошивки через devlink, используйте инструмент [QuantumPCI-Firmware-Tool](https://github.com/SiwaNetwork/QuantumPCI-Firmware-Tool):
...
Измените в файле `header/firmware.go`:
```go
var hdrMagic = [4]byte{'O', 'C', 'P', 'C'}
```

### Создание прошивки с заголовком
```bash
go build -o quantum-pci-ft
./quantum-pci-ft -input исходная_прошивка.bin \
    -output прошивка_с_заголовком.bin \
    -vendor 0xXXXX \
    -device 0xYYYY \
    -apply
```
...
```bash
sudo cp прошивка_с_заголовком.bin /lib/firmware/
sudo devlink dev flash pci/0000:XX:XX.X file прошивка_с_заголовком.bin
```

### Проверка результата
```bash
sudo devlink dev info pci/0000:XX:XX.X
lspci | grep "XX:XX.X"
```